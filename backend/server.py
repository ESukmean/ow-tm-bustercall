...
class AccountUpdate(Resource):
	def post(self, phone):
		...
		enabled = True if request.form['enabled'] == 'Y' else False
		client.smsList.update_one({'phoneNo': phone}, {'$set': {'enabled': enabled}})

		return {'result': True, 'msg': '적용되었습니다.'}

api.add_resource(AccountUpdate, '/api/<string:phone>/update')
